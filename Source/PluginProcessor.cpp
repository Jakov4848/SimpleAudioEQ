...
void SimpleEQAudioProcessor::updatePeakFilter(const ChainSettings& chainSettings) {
	...
	//	Create biquad coefficients for the peak filter
	auto peakCoefficients = juce::dsp::IIR::Coefficients<float>::makePeakFilter(
		getSampleRate(), chainSettings.peakFreq, chainSettings.peakQuality,
		juce::Decibels::decibelsToGain(chainSettings.peakGainInDecibels));

	//	Assign the coefficients to the peak filter in both left and right chains
...
SimpleEQAudioProcessor::createParameterLayout() {
	...
	layout.add(std::make_unique<juce::AudioParameterFloat>(
		...
		juce::NormalisableRange<float>(0.1, 10.f, 0.05f, 1.0f), 1.f));
	...
	layout.add(std::make_unique<juce::AudioParameterChoice>(
		"LowCut Slope", "LowCut Slope", choices, 0));

	layout.add(std::make_unique<juce::AudioParameterChoice>(
		"HighCut Slope", "HighCut Slope", choices, 0));
...
void SimpleEQAudioProcessor::prepareToPlay(double sampleRate,
	...
	//	Retrieve the chain settings from the APVTS
...
void SimpleEQAudioProcessor::processBlock(juce::AudioBuffer<float>& buffer,
	...
	auto lowCutCofficients = juce::dsp::FilterDesign<float>::
		designIIRHighpassHighOrderButterworthMethod(
	...
	updateCutFilter(leftLowCut, lowCutCofficients, chainSettings.lowCutSlope);
	updateCutFilter(rightLowCut, lowCutCofficients, chainSettings.lowCutSlope);
